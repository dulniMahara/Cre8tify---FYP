...
async function runMigration() {
    ...
    console.log("Connected. Fetching IDs...");

    try {
        const ids = await Product.find().select('_id');
        console.log(`Found ${ids.length} products to process in batches.`);

        for (let i = 0; i < ids.length; i++) {
            console.log(`Fetching product ${i + 1}/${ids.length} (${ids[i]._id})...`);
            const product = await Product.findById(ids[i]._id);
            if (!product) continue;
