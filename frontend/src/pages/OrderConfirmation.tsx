...
const OrderConfirmation = () => {
        ...
        ? [
            {
              ...
              basePrice: 1200,
              markup:
                customProduct.designerMarkup ||
                customProduct.markup ||
                Math.max(0, parseFloat(customProduct.price) - 1200 - 100 - 300),
              serviceFee: 100,
  ...
  const baseDeliveryFee = items.some((i: any) => i.type === "physical")
    ? 300
    : 0;
    ...
    try {
      ...
      const orderData = {
        orderItems: items.map((item: any) => ({
          ...
          product:
            (item._id || item.id) &&
            /^[0-9a-fA-F]{24}$/.test(String(item._id || item.id))
              ? item._id || item.id
              : undefined,
        })),
        totalPrice: total,
        ...
        status:
          paymentMethod === "bank" ? "Awaiting Verification" : "Processing",
      };
        ...
        <div style={leftCol}>
          <div style={sectionCard}>
            <div style={sectionHeader}>
              ...
              <button onClick={() => setIsEditModalOpen(true)} style={editBtn}>
                Edit
              </button>
            </div>
              ...
              {items.map((item: any) => (
                <div key={item.id} style={itemRow}>
                  <div
                    style={{
                      ...itemThumb,
                      padding: 0, // <--- THIS IS THE MAGIC FIX that stops the shrinking!
                      overflow: "hidden",
                      position: "relative",
                      display: "flex",
                      justifyContent: "center",
                  ...
                  >
                    {(() => {
                      // Attempt to find the matching physical item in the checkout array
                      const matchedPhys = items.find(
                        (p: any) =>
                          p.type === "physical" &&
                      ...
                      if (activeDesign) {
                        ...
                        const activeColor = (rawColor === "Choose Color" || !rawColor) ? "#ffffff" : rawColor;
                        ...
                        return (
                          <div
                            style={{
                              transform: "translateY(-15px)", // Centers the chest perfectly
                              width: "80px",
                            }}
                          >
                            <PerfectPreview
                  ...
                  style={{
                    ...
                    border:
                      paymentMethod === method.id
                        ...
                        : "1px solid #e2e8f0",
                    background:
                      paymentMethod === method.id ? "#f0f7ff" : "#fff",
                  }}
                >
                  <input
                    type="radio"
          ...
          <div style={summaryCard}>
            ...
            <div style={summaryRows}>
              ...
              <div style={summaryRow}>
                ...
                <span>LKR {formatPrice(baseDeliveryFee)}</span>
              </div>
              <div style={totalRow}>
        ...
        <div style={modalOverlay}>
          <div style={modalContent}>
            ...
            <div style={modalActions}>
              <button style={saveBtn} onClick={handleEditSave}>
                Save Changes
              </button>
              <button
                style={cancelBtn}
...
const shippingBody: React.CSSProperties = {
  display: "flex",
  flexDirection: "column",
  gap: "12px",
};
...
const itemThumb: React.CSSProperties = {
  width: "80px",
  height: "80px",
  objectFit: "contain",
  background: "#fff",
...
const paymentOptions: React.CSSProperties = {
  display: "flex",
  flexDirection: "column",
  gap: "12px",
};
...
const methodName: React.CSSProperties = {
  fontSize: "15px",
  fontWeight: "700",
  color: "#1e293b",
};
...
const bankInstructions: React.CSSProperties = {
  marginTop: "20px",
  padding: "20px",
  background: "#f8fafc",
  borderRadius: "15px",
...
const summaryCard: React.CSSProperties = {
  ...
  padding: "30px",
  color: "#fff",
  boxShadow: "0 10px 30px rgba(13, 55, 91, 0.2)",
};
...
const summaryRows: React.CSSProperties = {
  display: "flex",
  flexDirection: "column",
  gap: "15px",
};
