...
const TrackOrder = () => {
    ...
    const publicDirectionsUrl = `https://maps.google.com/maps?q=${encodeURIComponent(warehouseLocation)}&daddr=${encodeURIComponent(address + ", Sri Lanka")}&output=embed&t=m`;

    // 🚀 FIXED: Variables are now simple strings
    const deliveryStart = formatTrackDate(baseDate, 5, 'DATE_ONLY');
    const deliveryEnd = formatTrackDate(baseDate, 7, 'DATE_ONLY');

    const steps = [
        { title: "Order Placed", date: formatTrackDate(baseDate, 0), status: "complete", desc: "We've received your order." },
        { title: "Processing", date: formatTrackDate(baseDate, 1), status: "complete", desc: "Item packed at Colombo Hub." },
        { title: "Dispatched", date: formatTrackDate(baseDate, 2), status: "active", desc: "Handled by Cre8tify Logistics." },
        { title: "Out for Delivery", date: "Pending", status: "pending", desc: "Courier is on the way." },
        { title: "Delivered", date: `Expected ${deliveryStart}`, status: "pending", desc: "Enjoy your purchase!" },
    ];
    ...
    return (
        <div style={pageWrapper}>
            ...
            <header style={blueHeader}>
                <div style={backArea} onClick={() => navigate('/my-orders')}>
