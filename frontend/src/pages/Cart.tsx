...
const Cart = () => {
  ...
  const { cartItems, toggleSelect, toggleAll, updateQuantity, removeItem } =
    useCart();
  ...
  const deliveryFee = selectedItems.some(
    ...
    <div style={pageWrapper}>
      ...
      <div style={{ width: "100%", background: "#0d375b" }}>
        <div style={{ paddingLeft: "0px" }}>
          {" "}
          {/* This nudges only the Title/Back button */}
                ...
                .map((item: any) => (
                  <div key={item.id + item.size} style={itemRow}>
                    ...
                    <div style={productCell}>
                      ...
                      >
                        {(() => {
                          const matchedPhys = cartItems.find(
                            (p: any) =>
                              p.type === "physical" &&
                              (p.id === item.id || (item.title && p.title && item.title.includes(p.title)))
                          );

                          const activeDesign = item.frontDesign || item.image || matchedPhys?.frontDesign;
                          
                          if (activeDesign) {
                            const rawColor = item.tshirtColor || item.color || matchedPhys?.tshirtColor || matchedPhys?.color || "#ffffff";
                            const activeColor = rawColor === "Choose Color" ? "#ffffff" : rawColor;
                            const activePrintArea = item.frontPrintArea || matchedPhys?.frontPrintArea;
                            const activeBase = (item.baseImages && item.baseImages[0]) || (matchedPhys?.baseImages && matchedPhys?.baseImages[0]) || "/img/womenfront-mockup.png";
                            
                            return (
                              <div
                                style={{
                                  transform: "translateY(-20px)", // Centers the 100x100 Cart box
                                  width: "100px",
                                }}
                              >
                                <PerfectPreview
                                  mockupSrc={activeBase}
                                  maskSrc={activeBase}
                                  tshirtColor={activeColor}
                                  printArea={activePrintArea}
                                  frontDesign={activeDesign}
                                  containerWidth={100}
                                />
                              </div>
                            );
                          ...
                          return (
                            <img src="/img/placeholder.png" alt="Fallback" style={{ width: "100%", height: "100%", objectFit: "contain", padding: "10px" }} />
                          );
                        })()}
                      </div>
                            ...
                            style={{
                              color:
                                item.color === "Choose Color"
                                  ...
                                  : "#64748B",
                              fontWeight:
                                item.color === "Choose Color" ? "800" : "500",
                              cursor:
                                item.color === "Choose Color"
                                  ? "pointer"
                                  : "default",
                ...
                .map((item: any) => (
                  <div key={item.id} style={itemRow}>
                    ...
                    <div style={productCell}>
                      ...
                      >
                        {(() => {
                          // 1. Attempt to find the matching physical item in the cart to borrow its preview data
                          const matchedPhys = cartItems.find(
                            (p: any) =>
                              p.type === "physical" &&
                              (p.id === item.id || item.title.includes(p.title))
                          );

                          // 2. Safely merge data
                          const activeDesign = item.frontDesign || matchedPhys?.frontDesign;
                          
                          if (activeDesign) {
                            const activeColor = item.tshirtColor || item.color || matchedPhys?.tshirtColor || matchedPhys?.color || "#ffffff";
                            const activePrintArea = item.frontPrintArea || matchedPhys?.frontPrintArea;
                            const activeBase = (item.baseImages && item.baseImages[0]) || (matchedPhys?.baseImages && matchedPhys?.baseImages[0]) || "/img/womenfront-mockup.png";
                          ...
                          // 3. Fallback if no design graphic is available (prevents the "shirt-in-shirt" bug)
                          const imgUrl = item.image || item.img;
                          const safeImgUrl = imgUrl 
                            ? (imgUrl.startsWith("http") || imgUrl.startsWith("/") || imgUrl.startsWith("data:"))
                                ? imgUrl 
                                : `/img/${imgUrl}`
                            : "/img/placeholder.png";
                      ...
                      <img
                        src="/img/pdf.png"
                        style={{ width: "45px", margin: "0 30px", flexShrink: 0 }}
                        alt="PDF"
                      />
          ...
          <div style={cartSummaryArea}>
            <div style={selectAllControl}>
              <input
                ...
                checked={
                  cartItems.length > 0 &&
                  cartItems.every((i: any) => i.selected)
                }
            ...
            <div style={summaryCalculationBox}>
              ...
              <div style={summaryLine}>
                ...
                <span>LKR {deliveryFee.toLocaleString()}.00</span>
              </div>
              <div style={totalLine}>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (hasIncompleteItems) {
                    alert(
                      "Please select a Size and Color for all items before checking out!",
                    );
                    return;
                  }
...
const parallelEmptyWrapper: React.CSSProperties = {
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  width: "100%",
...
// Image Style
const emptyIconParallel: React.CSSProperties = {
  width: "220px",
  height: "auto",
  opacity: 0.9,
};
const emptyTextSide: React.CSSProperties = {
  textAlign: "left",
  maxWidth: "400px",
};
const emptyTitleParallel: React.CSSProperties = {
  fontSize: "32px",
  fontWeight: 900,
  color: "#0d375b",
  marginBottom: "15px",
...
const emptyDescParallel: React.CSSProperties = {
  fontSize: "15px",
  color: "#64748B",
  lineHeight: "1.6",
  marginBottom: "30px",
};
const startShoppingBtnParallel: React.CSSProperties = {
  background: "#0d375b",
  color: "#fff",
  padding: "15px 40px",
  borderRadius: "10px",
  ...
  border: "none",
  cursor: "pointer",
  boxShadow: "0 10px 25px rgba(13, 55, 91, 0.2)",
};

const blueHeader: React.CSSProperties = {
  padding: "20px 8%",
  display: "flex",
...
const backArea: React.CSSProperties = {
  display: "flex",
  alignItems: "center",
  gap: "10px",
  cursor: "pointer",
...
const cartHeading: React.CSSProperties = {
  fontSize: "28px",
  fontWeight: 900,
  margin: 0,
  color: "#0d375b",
...
const headerNudgeStyle: React.CSSProperties = {
  width: "100%",
  paddingLeft: "30px",
  background: "#0d375b",
};
...
const blueTableHeader: React.CSSProperties = {
  background: "#0d375b",
  color: "#fff",
  padding: "15px 30px",
  display: "flex",
...
const customCheck: React.CSSProperties = {
  width: "20px",
  height: "20px",
  cursor: "pointer",
  accentColor: "#0d375b",
...
const qtyCell: React.CSSProperties = {
  flex: 1.5,
  display: "flex",
  justifyContent: "center",
};
...
const qtyAction: React.CSSProperties = {
  background: "none",
  border: "none",
  cursor: "pointer",
  fontSize: "18px",
...
const cartSummaryArea: React.CSSProperties = {
  marginTop: "40px",
  display: "flex",
  justifyContent: "space-between",
...
const selectAllControl: React.CSSProperties = {
  display: "flex",
  alignItems: "flex-start",
  paddingTop: "8px",
};
const summaryCalculationBox: React.CSSProperties = {
  width: "380px",
  background: "#f8fafc",
  padding: "25px",
  borderRadius: "16px",
...
const emptyStateWrapper: React.CSSProperties = {
  display: "flex",
  flexDirection: "column",
  alignItems: "center",
  justifyContent: "center",
...
const emptyIconAnim: React.CSSProperties = {
  width: "150px",
  marginBottom: "15px",
  opacity: 0.9,
};
...
const startShoppingBtn: React.CSSProperties = {
  background: "#0d375b",
  color: "#fff",
  padding: "15px 40px",
  borderRadius: "30px",
...
const footerLinksGroup: React.CSSProperties = {
  display: "flex",
  gap: "30px",
  fontWeight: 600,
};
