...
const MarketplaceOperations = () => {
  ...
  const [selectedDesignerHistory, setSelectedDesignerHistory] =
    useState<any>(null);
  const [showDesignerHistoryModal, setShowDesignerHistoryModal] =
    useState(false);
    ...
    try {
      if (tab === "Payouts") {
        const [sumRes, desRes] = await Promise.all([
          fetch(`${API_URL}/api/admin/financial/summary`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
          fetch(`${API_URL}/api/admin/financial/designers`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
      ...
      if (tab === "Approvals") endpoint = "/api/products/admin/pending";
        ...
        endpoint = "/api/requests?type=customization";
  ...
  const handleRefund = async (id: string) => {
    if (
      !window.confirm(
        "Are you sure you want to refund this order? This will update financial records.",
      )
    )
      return;
    const token = getToken("admin");
  ...
  const handleSaveCategory = async () => {
    ...
    const Url = editingCategory
      ? `${API_URL}/api/base-products/${editingCategory._id}`
      : `${API_URL}/api/base-products`;
  ...
  const handleDeleteCategory = async (id: string) => {
    if (
      !window.confirm(
        "Delete this base template? This action cannot be undone.",
      )
    )
      return;
    const token = getToken("admin");
          ...
          {tshirtColor && (
            <div
              style={{
                ...
                display:
                  tshirtColor.toLowerCase() === "#ffffff" ? "none" : "block",
          ...
          >
            {imageLayers.map((layer: any) => (
              <img
                key={layer.id}
                src={
                  layer.src.startsWith("/uploads")
                    ? `http://localhost:5000${layer.src}`
                    : layer.src
                }
                alt="Design Layer"
                style={{
                  ...
                  mixBlendMode:
                    tshirtColor && tshirtColor.toLowerCase() !== "#ffffff"
                      ? "multiply"
                      : "normal",
                  opacity: 0.95,
                  width: "auto",
              ...
              >
                {t.styleId === "default" ? (
                  ...
                  <>
                    ...
                    {!["style-wave", "style-stack", "style-fish"].includes(
                      t.styleId || "",
                    ) && (
                      <AdminCurvedText
                        id={t.id}
    ...
    >
      {approvals.map((product: any) => (
        <div key={product._id} style={cardStyle}>
          <div
            style={{ ...imageContainerStyle, height: "220px", padding: "15px" }}
          >
            <div
              style={{
                position: "relative",
          ...
          <div style={{ padding: "15px" }}>
            <h3
              style={{ margin: "0 0 5px 0", fontSize: "15px", color: "white" }}
            >
              {product.title}
            </h3>
            <p
  ...
  const renderCategories = () => {
    ...
    return (
      <div>
        {groups.map((group) => {
          const groupData = categories.filter(
            (item: any) => (item.category || "Unisex") === group,
          );
                ...
                {groupData.map((item: any) => (
                  <div
                    ...
                    style={{
                      ...
                      boxShadow: "0 4px 20px rgba(0,0,0,0.3)",
                      width: "210px",
                      margin: group === "Kids" ? "0 15px" : "0 auto",
                    }}
                  >
                    <div
                      style={{
                    ...
                    >
                      <img
                        src={item.image || "/img/womenfront-mockup.png"}
                        style={{
                          width: "100%",
                          height: "100%",
                          objectFit: "cover",
                    ...
                    <div style={{ padding: "15px" }}>
                      ...
                      >
                        {item.sizes ? item.sizes.length : 4} sizes •{" "}
                        {item.colors ? item.colors.length : 5} colors
                      </p>
                      <div style={{ display: "flex", gap: "8px" }}>
                        <button
                          onClick={() => {
      ...
      <div>
            ...
            {statuses.map((s) => (
              <button
                ...
                style={{
                  ...
                  boxShadow:
                    filterStatus === s
                      ? "0 4px 10px rgba(56, 189, 248, 0.2)"
                      : "none",
                }}
              >
                {s}
        ...
        <div style={{ ...tableCardStyle, marginTop: "15px" }}>
                      ...
                      >
                        ...
                        >
                          <PerfectPreview
                            ...
                            maskSrc="/img/womenfront-mockup.png"
                            tshirtColor={
                              order.orderItems?.[0]?.tshirtColor || "#ffffff"
                            }
                            printArea={order.orderItems?.[0]?.frontPrintArea}
                            frontDesign={order.orderItems?.[0]?.frontDesign}
                            containerWidth={50}
                          />
                        </div>
                      </div>
                      ...
                      >
                        ...
                        {order.paymentMethod === "bank" &&
                          order.paymentSlipUrl && (
                            <a
                              href={
                                order.paymentSlipUrl.startsWith("http")
                                  ? order.paymentSlipUrl
                                  : `http://localhost:5000${order.paymentSlipUrl}`
                              }
                              target="_blank"
                      ...
                      {(() => {
                        const isDigitalOnly =
                          ...
                          order.orderItems.every(
                            (i: any) =>
                              i.name?.includes("(Digital)") ||
                              i.image?.includes("digital_download_icon"),
                          );
          ...
          {filteredOrders.length === 0 && (
            ...
            >
              <img
                src="/img/no-orders.png"
                style={{ width: "50px", opacity: 0.3, marginBottom: "15px" }}
                alt=""
        ...
        {showOrderDetailsModal && selectedOrderDetails && (
          <div style={modalOverlayStyle}>
            <div style={{ ...modalContentStyle, maxWidth: "600px" }}>
              ...
              >
                <h2 style={{ fontSize: "20px", color: "#38bdf8", margin: 0 }}>
              ...
              >
                ...
                >
                  <div>
                    ...
                    <div style={{ fontWeight: "bold", color: "white" }}>
                      ...
                      {selectedOrderDetails._id
                        .substring(selectedOrderDetails._id.length - 8)
                        .toUpperCase()}
                    </div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    >
                      {selectedOrderDetails.paymentMethod === "bank"
                        ? "Bank Deposit"
                        : selectedOrderDetails.paymentMethod === "card"
                          ? "Card"
                          : selectedOrderDetails.paymentMethod === "sandbox"
                            ? "Sandbox"
                            : selectedOrderDetails.paymentMethod || "Unknown"}
                      {selectedOrderDetails.paymentMethod === "bank" &&
                        selectedOrderDetails.paymentSlipUrl && (
                          <a
                            href={
                              selectedOrderDetails.paymentSlipUrl.startsWith(
                                "http",
                              )
                                ? selectedOrderDetails.paymentSlipUrl
                                : `http://localhost:5000${selectedOrderDetails.paymentSlipUrl}`
                            }
                            target="_blank"
                            rel="noopener noreferrer"
                            style={{
                ...
                {selectedOrderDetails.orderItems?.map(
                  (item: any, idx: number) => (
                    ...
                    >
                      ...
                      >
                        <div>
                          Base: LKR {(item.basePrice || 1200).toLocaleString()}
                        </div>
                        <div>
                          Markup: LKR {(item.markup || 0).toLocaleString()}
                        </div>
                        <div>
                          Service Fee: LKR{" "}
                          {(item.serviceFee || 100).toLocaleString()}
                        </div>
                        {item.isCustom && (
                          <div style={{ color: "#fbbf24", fontWeight: "bold" }}>
                            Customization: LKR{" "}
                            {(item.customizationFee || 300).toLocaleString()}
                          </div>
                        )}
                      </div>
                    </div>
              ...
              >
                {!(
                  ...
                  selectedOrderDetails.orderItems.every(
                    (i: any) =>
                      i.name?.includes("(Digital)") ||
                      i.image?.includes("digital_download_icon"),
                  )
                  ...
                  >
                    ...
                    <span>
                      LKR{" "}
                      {(
                        selectedOrderDetails.shippingFee || 300
                      ).toLocaleString()}
                    </span>
                  </div>
                )}
                ...
                >
                  ...
                  <span>
                    LKR {selectedOrderDetails.totalPrice.toLocaleString()}
                  </span>
                </div>
                ...
                >
                  ...
                  >
                    ...
                    <span>
                      LKR{" "}
                      {(
                        selectedOrderDetails.platformProfit || 0
                      ).toLocaleString()}
                    </span>
                  </div>
                  <div
                  ...
                  >
                    ...
                    <span>
                      LKR{" "}
                      {(
                        selectedOrderDetails.designerEarnings || 0
                      ).toLocaleString()}
                    </span>
                  </div>
                </div>
      ...
      <div style={{ padding: "0 20px", textAlign: "left" }}>
        ...
        >
          ...
          >
            ...
            <div
              style={{ fontSize: "32px", fontWeight: "800", color: "#f8fafc" }}
            >
            ...
            >
              <span style={{ color: "#22c55e" }}>↑ 100%</span> Across all paid
              orders
            </div>
          </div>
          ...
          >
            ...
            <div
              style={{ fontSize: "12px", marginTop: "12px", color: "#64748b" }}
            >
              Owed to designers
            </div>
          </div>
        ...
        >
          ...
          >
            Designer Earnings & Settlements
          </h3>
          <div style={{ fontSize: "12px", color: "#94a3b8" }}>
            Total Designers: {designerPayouts.length}
          </div>
        </div>
        ...
        <div style={tableCardStyle}>
            ...
            <tbody>
              {designerPayouts.map((d: any) => (
                <tr key={d.id} style={tdRowStyle}>
                  ...
                  <td style={tdStyle}>
                    LKR {d.totalEarned.toLocaleString()}.00
                  </td>
                  <td style={tdStyle}>
                    LKR {d.alreadyPaid.toLocaleString()}.00
                  </td>
                  <td
                    style={{
                      ...tdStyle,
                      color: d.balance > 0 ? "#f43f5e" : "#10b981",
                      fontWeight: "700",
                    }}
                  >
                    LKR {d.balance.toLocaleString()}.00
                  </td>
                  <td style={tdStyle}>
                    <div style={{ display: "flex", gap: "8px" }}>
                      <button
                      ...
                      <button
                        onClick={async () => {
                          ...
                          const res = await fetch(
                            `${API_URL}/api/admin/financial/designers/${d.id}`,
                            {
                              headers: { Authorization: `Bearer ${token}` },
                            },
                          );
          ...
          {designerPayouts.length === 0 && (
            <p
              style={{ padding: "40px", color: "#64748b", textAlign: "center" }}
            >
              No designer records available.
            </p>
          )}
        </div>
        ...
        {showDesignerHistoryModal && selectedDesignerHistory && (
          <div style={modalOverlayStyle}>
            <div style={{ ...modalContentStyle, maxWidth: "700px" }}>
              ...
              >
                <h2 style={{ fontSize: "20px", color: "#38bdf8", margin: 0 }}>
                  Transaction History: {selectedDesignerHistory.designer?.name}
                </h2>
                <button
                  onClick={() => setShowDesignerHistoryModal(false)}
                  style={{
                    background: "none",
                    border: "none",
                    color: "#94a3b8",
              ...
              >
                ...
                >
                  ...
                  <div style={{ fontSize: "18px", fontWeight: "bold" }}>
                    LKR{" "}
                    {selectedDesignerHistory.summary?.totalEarned.toLocaleString()}
                  </div>
                </div>
                <div
                  style={{
                ...
                >
                  ...
                  <div style={{ fontSize: "18px", fontWeight: "bold" }}>
                    LKR{" "}
                    {selectedDesignerHistory.summary?.alreadyPaid.toLocaleString()}
                  </div>
                </div>
                <div
                  style={{
                ...
                >
                  ...
                  >
                    LKR{" "}
                    {selectedDesignerHistory.summary?.balance.toLocaleString()}
                  </div>
                </div>
              </div>
              ...
              <div style={{ maxHeight: "400px", overflowY: "auto" }}>
                <table style={tableStyle}>
                  ...
                  <tbody>
                    {selectedDesignerHistory.history?.map(
                      (h: any, idx: number) => (
                        <tr
                          key={idx}
                          style={{
                        ...
                        >
                          ...
                          <td
                            style={{
                              ...
                              textAlign: "right",
                              fontWeight: "bold",
                              color: h.amount > 0 ? "#22c55e" : "#f43f5e",
                            }}
                          >
                            {h.amount > 0 ? "+" : ""}
            ...
            <tbody>
              {allOrders.slice(0, 10).map((order: any) => (
                <tr key={order._id} style={tdRowStyle}>
                  ...
                  <td
                    style={{ ...tdStyle, fontWeight: "700", color: "#38bdf8" }}
                  >
                    #CR8-
                  ...
                  <td style={{ ...tdStyle, fontWeight: "600" }}>
                    LKR {order.totalPrice.toLocaleString()}.00
                  </td>
                  <td style={tdStyle}>
                    <span
                      style={{
                      ...
                      style={{
                        ...
                        background:
                          req.status === "Completed"
                            ...
                            : "rgba(56, 189, 248, 0.1)",
                        color:
                          req.status === "Completed" ? "#22c55e" : "#38bdf8",
                        border:
                          req.status === "Completed"
                            ? "1px solid rgba(34, 197, 94, 0.2)"
  ...
  const renderCustomizations = () => {
    const pendingCustomizations = requests.filter(
      (r) => r.requestType === "customization" && r.status === "Pending",
    );
      ...
      <div style={{ padding: "0 0 50px 0" }}>
        ...
        >
          {pendingCustomizations.map((req: any) => {
            ...
            const baseDesignerCharge = getSettingValue(
              "base_designer_charge",
              500,
            );
            const designerBonus = getSettingValue("customization_bonus", 100);
            const platformFee = getSettingValue(
              "platform_customization_fee",
              300,
            );
                ...
                <div style={{ ...imageContainerStyle, height: "180px" }}>
                  ...
                  >
                    <PerfectPreview
                      mockupSrc="/img/womenfront-mockup.png"
                      maskSrc="/img/womenfront-mockup.png"
                      tshirtColor={req.tshirtColor || req.color || "#ffffff"}
          ...
          {pendingCustomizations.length === 0 && (
            <p style={{ color: "#64748b", fontSize: "14px" }}>
              No pending user customizations.
            </p>
          )}
        </div>
      </div>
  ...
  return (
    ...
    >
      ...
      >
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...tabBtnStyle,
                  border: "1px solid rgba(255,255,255,0.1)",
                  background: activeTab === tab ? "#38bdf8" : "#0f172a",
        ...
        {loading && (
          <p style={{ color: "#0d375b", fontWeight: "bold" }}>
        ...
        {(() => {
          ...
          const globalSession = JSON.parse(
            localStorage.getItem("userInfo") || "{}",
          );
          if (
            !adminSession &&
            globalSession.role &&
            globalSession.role !== "admin"
          ) {
            return (
              <div
                style={{
              ...
              >
                <h3 style={{ color: "#be123c", margin: "0 0 10px 0" }}>
                  Admin Authorization Required
                </h3>
                <p style={{ color: "#e11d48", fontSize: "14px", margin: 0 }}>
                  You are currently logged in as a{" "}
                  <strong>{globalSession.role}</strong>. Please log out and sign
                  in with an Admin account to manage orders and operations.
                </p>
              </div>
            );
          }
      ...
      {showRejectModal && (
        <div style={modalOverlayStyle}>
          <div
            style={{ ...modalContentStyle, width: "400px", maxWidth: "90%" }}
          >
            <h2
              style={{
                fontSize: "18px",
            ...
            <div style={{ display: "flex", gap: "10px" }}>
              <button
                onClick={() =>
                  handleStatusUpdate(selectedProduct._id, "Rejected", reason)
                }
      ...
      {showCategoryModal && (
        <div style={modalOverlayStyle}>
          <div style={{ ...modalContentStyle, maxWidth: "600px" }}>
            <h2
              style={{ fontSize: "18px", color: "white", marginBottom: "20px" }}
            >
              {editingCategory ? "Edit Base T-Shirt" : "Add Base T-Shirt"}
            </h2>
            <div
              style={{
                display: "grid",
            ...
            >
              ...
              <div>
                ...
                >
                  ...
                  <option value="Women" style={{ background: "#1e293b" }}>
                    Women
                  </option>
                  <option value="Men" style={{ background: "#1e293b" }}>
                    Men
                  </option>
                  <option value="Kids" style={{ background: "#1e293b" }}>
                    Kids
                  </option>
                </select>
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCategoryForm({
                      ...categoryForm,
                      basePrice: e.target.value ? parseInt(e.target.value) : "",
                    })
                  }
                  style={inputStyle}
                />
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="/img/model-placeholder.png"
                  onChange={(e) =>
                ...
                {PREDEFINED_COLORS.map((color) => (
                  <div
                    ...
                    style={{
                      padding: "4px 8px",
                      borderRadius: "20px",
                      border: `1px solid ${categoryForm.colors.includes(color.name) ? "#38bdf8" : "#cbd5e1"}`,
                      background: categoryForm.colors.includes(color.name)
                        ? "#38bdf8"
                        : "white",
                      cursor: "pointer",
                      display: "flex",
                ...
                {PREDEFINED_SIZES.map((size) => (
                  <div
                    ...
                    style={{
                      padding: "4px 12px",
                      borderRadius: "6px",
                      border: `1px solid ${categoryForm.sizes.includes(size) ? "#0d375b" : "#cbd5e1"}`,
                      background: categoryForm.sizes.includes(size)
                        ? "#0d375b"
                        : "white",
                      color: categoryForm.sizes.includes(size)
                        ? "white"
                        : "#475569",
                      cursor: "pointer",
                      fontSize: "11px",
            ...
            <div style={{ display: "flex", gap: "10px" }}>
              <button
                ...
                style={{ ...approveBtnStyle, flex: 1, background: "#0284c7" }}
              >
                Save Configuration
              </button>
              <button
      ...
      {showPayoutModal && (
        <div style={modalOverlayStyle}>
          <div style={{ ...modalContentStyle, width: "450px" }}>
            ...
            >
              Settle earnings for <strong>{payoutForm.name}</strong>
            </p>
            <div
              style={{
                display: "flex",
            ...
            >
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setPayoutForm({ ...payoutForm, note: e.target.value })
                  }
...
const UserManagement = () => {
  ...
  const handlePasswordReset = async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/api/admin/users/${selectedUser._id}/reset-password`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
        ...
        {loading ? (
          ...
          <div style={tableCardStyle}>
            <table style={tableStyle}>
              <thead>
                <tr style={thStyle}>
                  <th style={{ paddingLeft: "150px", verticalAlign: "middle" }}>
                    User
                  </th>
                  <th style={{ paddingLeft: "55px", verticalAlign: "middle" }}>
                    Role
                  </th>
                  <th style={{ paddingLeft: "18px", verticalAlign: "middle" }}>
                    Join Date
                  </th>
                    ...
                    >
                      <span
                        style={{
                          ...
                          fontWeight: "bold",
                          textTransform: "uppercase",
                          background: getStatusBadgeStyle(
          ...
          <div style={{ ...modalContentStyle, maxWidth: "500px" }}>
            ...
            >
              <h2 style={{ fontSize: "18px", color: "#38bdf8", margin: 0 }}>
                Security Logs: {selectedUser.name}
              </h2>
              <button
            ...
            <div style={{ maxHeight: "400px", overflowY: "auto" }}>
              {selectedUser.securityLogs &&
              selectedUser.securityLogs.length > 0 ? (
                selectedUser.securityLogs.map((log: any, idx: number) => (
                  ...
                  >
                    ...
                    <div style={{ color: "#64748b", fontSize: "12px" }}>
                      IP: {log.ip} | Location: {log.location || "Unknown"}
                    </div>
                  </div>
                ))
              ) : (
            ...
            <button
              type="button"
              onClick={() => setShowLogsModal(false)}
              style={{ ...cancelBtnStyle, width: "100%", marginTop: "20px" }}
            >
              Close
            </button>
          </div>
      ...
      >
          ...
          <div style={premiumStatCardStyle}>
            ...
            <div style={statContentStyle}>
              ...
              <div style={statValueStyle}>
                LKR {(financials.totalRevenue || 0).toLocaleString()}
              </div>
            </div>
          </div>
          <div style={premiumStatCardStyle}>
            ...
            <div style={statContentStyle}>
              ...
              <div style={statValueStyle}>
                {financials.totalOrders || 0} ORDERS
              </div>
            </div>
          </div>
          <div style={premiumStatCardStyle}>
            ...
            <div style={statContentStyle}>
              ...
              <div style={statValueStyle}>
                LKR {(financials.platformProfit || 0).toLocaleString()}
              </div>
            </div>
          </div>
          <div style={premiumStatCardStyle}>
        ...
        <div style={sectionHeaderStyle}>
          <span style={sectionTitleStyle}>
            Administrative Tasks & Platform Health
          </span>
          <div style={sectionDividerStyle} />
        </div>
        <div
            ...
            >
              ...
              {[0, 0.25, 0.5, 0.75, 1].map((p, idx) => (
                <div
                  ...
                  style={{ bottom: `${p * 200 + 30}px`, left: 0 }}
                />
              ))}
              ...
              {salesData.length > 0 ? (
                salesData.map((day: any, i: number) => {
                  const maxRev = Math.max(
                    ...salesData.map((d: any) => d.totalRevenue),
                    1,
                  );
                      ...
                      <img
                        src={
                          ...
                          product.mockupImages.length > 0
                            ? product.mockupImages[0].startsWith("/uploads")
                              ? `http://localhost:5000${product.mockupImages[0]}`
        ...
        >
          <h3
            style={{ color: "#f59e0b", fontSize: "16px", marginBottom: "25px" }}
          >
            User Growth Trend
          </h3>
          <div
          ...
          >
            ...
            {[0, 0.33, 0.66, 1].map((p, idx) => (
              <div
                key={idx}
                className="analytics-grid-line"
                style={{ bottom: `${p * 120 + 30}px`, left: 0 }}
              />
            ))}
            ...
            {userGrowth.length > 0 ? (
              userGrowth.map((day: any, i: number) => {
                const maxUsers = Math.max(
                  ...userGrowth.map((d: any) => d.newUsers),
                  1,
                );
  ...
  return (
    ...
    >
      <img
        ...
        alt={title}
      />
      <h2 style={{ color: "white", fontSize: "16px", marginBottom: "10px" }}>
        {title}
      </h2>
      <p
        style={{
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Monitor platform activity, curate designs, and manage payouts
          natively.
        </p>
      </div>
