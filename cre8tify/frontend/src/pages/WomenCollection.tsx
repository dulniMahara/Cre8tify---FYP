                    ...
                    <div style={{ padding: '0 60px 80px 60px' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '30px' }}>
                            {displayProducts.map((item: any) => ( 
                                <div key={item.id} className="product-card" style={{ background: 'white', padding: '15px', borderRadius: '24px', boxShadow: '0 4px 20px rgba(0,0,0,0.03)', border: '1px solid #f1f5f9', transition: 'all 0.3s ease', cursor: 'pointer', width: '100%', position: 'relative', overflow: 'visible' }}>
                                    <div style={{ background: '#f8fafc', borderRadius: '18px', display: 'flex', justifyContent: 'center', marginBottom: '15px', height: '340px', alignItems: 'center', overflow: 'hidden' }}>
                                        <img src={item.img} alt={item.title} style={{ maxWidth: '85%', maxHeight: '85%', objectFit: 'contain', filter: 'drop-shadow(0 15px 25px rgba(0,0,0,0.08))', transform: `scale(${item.scale || 1})` }} />
                                    </div>
                                    <div style={{ padding: '0 10px' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '5px' }}>
                                            <div style={{ flex: 1 }}>
                                                <div style={{ fontSize: '11px', fontStyle: 'italic', color: '#94a3b8', marginBottom: '2px' }}>Artisa LK</div>
                                                <h3 style={{ fontSize: '26px', fontWeight: '800', margin: '0', color: '#1e293b', lineHeight: '1.2' }}>{item.title}</h3>
                                            </div>
                                            <span onClick={(e) => { e.stopPropagation(); navigate(`/product/${item.id}`); }} style={{ fontSize: '11px', color: '#64748b', fontStyle: 'italic', textDecoration: 'underline', cursor: 'pointer', marginLeft: '10px' }}>View Details</span>
                                        </div>
                                        <div style={{ fontSize: '18px', fontWeight: '800', color: '#ef4444', margin: '12px 0' }}>LKR {item.price.toLocaleString()}.00</div>
